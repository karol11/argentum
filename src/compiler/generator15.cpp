...
struct Generator : ast::ActionScanner {
		...
		, layout("")
	{
		...
		null_ptr = llvm::Constant::getNullValue(ptr_type);
		...
		fn_copy_weak_field = llvm::Function::Create(
			...
			"copy_weak_field",
			*module);
		fn_copy_fixer_type = llvm::FunctionType::get(ptr_type, { ptr_type }, false)->getPointerTo();
		fn_reg_copy_fixer = llvm::Function::Create(
			llvm::FunctionType::get(void_type, { ptr_type, fn_copy_fixer_type }, false),
			llvm::Function::ExternalLinkage,
			"reg_copy_fixer",
			*module);
	...
	void build_retain(llvm::Value* ptr, bool is_weak) {
		if (is_weak)
			builder->CreateCall(fn_retain_weak->getType(), fn_retain_weak, { ptr });
		else
			builder->CreateCall(fn_retain->getType(), fn_retain, { ptr });
	}
	void build_release(llvm::Value* ptr, bool is_weak) {
		if (is_weak)
			builder->CreateCall(fn_relase_weak->getType(), fn_relase_weak, { ptr });
		else 
			builder->CreateCall(fn_release->getType(), fn_release, { ptr });
	}
	...
	void on_const_string(ast::ConstString& node) override {
		result->data = create_call(builder, classes[ast->string_cls].constructor, {});
		builder->CreateStore(
			builder->CreateGlobalStringPtr(node.value),
			cast_to(builder->CreateStructGEP(int_type, result->data, 2), tp_byte_ptr->getPointerTo()));
	...
	void compile_fn_body(ast::MkLambda& node, llvm::Type* closure_ptr_type = nullptr) {
		...
		if (!node.captured_locals.empty()) {
			...
			if (parent_capture_ptr) {
				builder->CreateStore(parent_capture_ptr, builder->CreateStructGEP(parent_capture_ptr, capture, 0));
			}
		}
		...
		for (auto& p : node.names) {
			...
			if (p->captured) {
				auto addr = builder->CreateStructGEP(p_val->getType(), capture, capture_offsets[p]);
	...
	llvm::Function* compile_function(ast::MkLambda& node, string name, llvm::Type* closure_ptr_type) {
		...
		current_function = llvm::Function::Create(
			static_cast<llvm::FunctionType*>(lambda_type->getElementType(1)->getPointerElementType()),
			llvm::Function::InternalLinkage,
			name,
			module.get());
	...
	Val handle_block(ast::Block& node, Val parameter) {
			...
			if (l->is_mutable || l->captured) {
				...
				if (l->captured) {
					addr = builder->CreateStructGEP(
						initializer.data->getType()->getPointerTo(),
						capture_ptrs.front(),
						capture_offsets[l]);
				}
		...
		for (auto& p : node.names) {
			if (temp_var == p) { // result is locked by the dying temp ptr.
				...
				if (p->is_mutable) {
					build_release(create_load(builder, val_iter->data), is_weak(p->type));
				} else {
		...
		if (auto calle_as_method = dom::strict_cast<ast::MakeDelegate>(node.callee)) {
			...
			if (method->cls->is_interface) {
				auto entry_point = builder->CreateCall(
					dispatcher_fn_type,
					builder->CreateLoad(ptr_type, receiver),
					{ builder->getInt64(classes[method->cls].interface_ordinal | m_info.ordinal) });
				result->data = builder->CreateCall(m_info.type, entry_point, move(params));
			} else {
				result->data = builder->CreateCall(
					...
					builder->CreateLoad(  // load ptr to fn
						...
						builder->CreateConstGEP2_32(
							classes[method->cls].vmt->getPointerTo(),
							builder->CreateLoad(ptr_type, receiver),
							-1, m_info.ordinal)),
					move(params));
			}
		...
		for (int d = int(captures.size()) - 1;; d--) {
			...
			if (++ptr_index >= capture_ptrs.size())
				capture_ptrs.push_back(
					create_load(builder,
						create_struct_gep(builder, capture_ptrs.back(), { 0 })));
		}
		return locals[var] = create_struct_gep(builder, capture_ptrs[ptr_index], capture_offsets[var]);
	}
	...
	void on_set(ast::Set& node) override {
		*result = make_retained_or_non_ptr(compile(node.val));
		...
		if (is_ptr(node.var->type)) {
			...
			build_release(create_load(builder, addr), is_weak(node.var->type));
			builder->CreateStore(
				cast_to(
					result->data,
	...
	void on_get_field(ast::GetField& node) override {
		...
		result->data = create_load(builder,
			create_struct_gep(builder, base.data, node.field->offset));
		if (is_ptr(node.type())) {
	...
	void on_set_field(ast::SetField& node) override {
		*result = make_retained_or_non_ptr(compile(node.val));
		if (is_ptr(node.type())) {
			auto base = compile(node.base);
			auto addr = create_struct_gep(builder, base.data, node.field->offset);
			build_release(create_load(builder, addr), is_weak(node.type()));
	...
	void on_mk_instance(ast::MkInstance& node) override {
		result->data = create_call(builder, classes[node.cls].constructor, {});
		result->lifetime = Val::Retained{};
	}
		...
		*result = compile(node.p[0]);
		...
		if (cls->is_interface) {
			...
			auto id = builder->CreateCall(
				dispatcher_fn_type,
				create_load(builder, create_struct_gep(builder, result->data, 0)),
				{ interface_ordinal });
			*result = compile_if(
				*result_type,
		...
		auto vmt_ptr = create_load(builder, create_struct_gep(builder, result->data, 0));
	...
	void on_eq(ast::EqOp& node) override {
		struct Comparer : ast::TypeMatcher {
			...
			void on_optional(ast::TpOptional& type) override {
				...
				OptComparer opt_matcher{ *this };
				type.wrapped->match(opt_matcher);
			}
		...
		Comparer type_comparer{ lhs.data, rhs.data, *this };
		node.p[0]->type()->match(type_comparer);
	...
	void on_mk_weak(ast::MkWeakOp& node) override {
		...
		auto src = compile(node.p);
		result->data = builder->CreateBitOrPointerCast(
			create_call(builder, fn_mk_weak, {
	...
	void on_deref_weak(ast::DerefWeakOp& node) override {
		auto src = compile(node.p);
		result->data = builder->CreateBitOrPointerCast(
			create_call(builder, fn_deref_weak, {
		...
		llvm::Value* interface_and_method)
	{
		...
		if (best.spread == vmts.size()) {  // exact match
			...
			return create_load(&builder,
				builder.CreateGEP(
					void_ptr_type->getPointerTo(),
	...
	void make_fn_retain() {
		...
		auto counter_addr = create_struct_gep(&b,
			...
			1);
		auto counter = create_load(&b, counter_addr);
		...
		auto wb_counter_addr = create_struct_gep(&b, b.CreateBitOrPointerCast(counter, weak_block_ptr), 2);
		b.CreateStore(
			b.CreateAdd(
				create_load(&b, wb_counter_addr),
				llvm::ConstantInt::get(tp_int_ptr, runtime::CTR_STEP)),
			wb_counter_addr);
		b.CreateBr(bb_null);
		b.SetInsertPoint(bb_null);
	...
	llvm::orc::ThreadSafeModule build() {
		...
		for (auto& cls : ast->classes) {
			...
			if (base_info)
				create_call(&builder, base_info->initializer, { info.initializer->arg_begin() });
			...
			for (auto& field : cls->fields) {
				builder.CreateStore(
					...
					create_struct_gep(&builder, result, field->offset));
			}
			builder.CreateRetVoid();
			// Constructor
			builder.SetInsertPoint(llvm::BasicBlock::Create(*context, "", info.constructor));
			result = create_call(&builder, fn_allocate, { builder.getInt64(layout.getTypeAllocSize(info.fields)) });
			create_call(&builder, info.initializer, { result });
			auto typed_result = builder.CreateBitOrPointerCast(result, info.fields->getPointerTo());
			builder.CreateStore(info.dispatcher, create_struct_gep(&builder, typed_result, 0));
			...
			if (special_copy_and_dispose.count(cls) == 0) {
				...
				if (base_info)
					create_call(&builder, base_info->dispose, { info.dispose->getArg(0) });
			...
			if (special_copy_and_dispose.count(cls) == 0) {
				...
				if (base_info)
					create_call(&builder, base_info->copier,
				...
				for (auto& f : cls->fields) {
					...
					if (is_weak(type)) {
						create_call(&builder, fn_copy_weak_field, {
							create_struct_gep(&builder, dst, f->offset),
							create_load(&builder, create_struct_gep(&builder, src, f->offset)) });
					} else if (is_ptr(type)) {
						builder.CreateStore(
							create_call(&builder, fn_copy_object_field, {
								cast_to(
									create_load(&builder, create_struct_gep(&builder, src, f->offset)),
									obj_ptr) }),
							cast_to(
