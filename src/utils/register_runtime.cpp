...
#define FN(A) (void(*)())(nullptr)
	...
	auto own_to_object = ast.get_own(ast.object);
	ast.mk_fn("getAtArray", FN(ag_fn_sys_getAtArray), opt_ref_to_object, { ast.get_conform_ref(ast.own_array), ast.tp_int64() });
	ast.mk_fn("setAtArray", FN(ag_fn_sys_setAtArray), ref_to_object, { ast.get_ref(ast.own_array), ast.tp_int64(), own_to_object });
	ast.mk_fn("setOptAt", FN(ag_fn_sys_setOptAt), new ast::ConstVoid, { ast.get_ref(ast.own_array), ast.tp_int64(), ast.tp_optional(own_to_object) });
